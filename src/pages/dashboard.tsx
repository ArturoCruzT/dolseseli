...
// ═══════════════════════════════════════════════════════
// PRICING CONFIG — Costos según plan
// ═══════════════════════════════════════════════════════
const PRICING = {
  free: { noRsvpCost: 10, perGuestCost: 1 },
  pro:  { noRsvpCost: 50, perGuestCost: 2 },
};

type PlanType = 'free' | 'pro';

const getPricing = (plan: string) => {
  return plan === 'pro' ? PRICING.pro : PRICING.free;
};
...
export default function Dashboard() {
  ...
  // ─── Cargar créditos y plan frescos desde DB ───────────
  ...
  // ═══════════════════════════════════════════════════════
  // PUBLICAR — Costos dinámicos según plan
  // ═══════════════════════════════════════════════════════
  const handlePublishClick = async (invitation: any) => {
    if (!user) return;
    await refreshUser();

    const pricing = getPricing(userPlan);
    const hasRSVP = invitation.features?.rsvp;
    const guestCount = guestCounts[invitation.id] || 0;
    const cost = hasRSVP ? guestCount * pricing.perGuestCost : pricing.noRsvpCost;
    ...
    if (hasRSVP) {
      ...
      if (cost > userCredits) {
        alert(`⚠️ No tienes suficientes créditos.\n\nTienes ${userCredits} créditos y necesitas ${cost} (${pricing.perGuestCost} × ${guestCount} invitados).\n\nCompra más créditos en Planes.`);
        return;
      }
    } else {
      if (userCredits < pricing.noRsvpCost) {
        alert(`⚠️ Necesitas ${pricing.noRsvpCost} créditos para publicar sin confirmación.\n\nTienes ${userCredits} créditos.`);
        return;
      }
    }
  ...
  const confirmPublish = async () => {
    ...
    try {
      ...
      const hasRSVP = invitation.features?.rsvp;
      const guestCount = guestCounts[invitation.id] || 0;
      const creditsToConsume = hasRSVP ? guestCount * pricing.perGuestCost : pricing.noRsvpCost;
      ...
      if (hasRSVP) {
        const { data: guests } = await supabase
          ...
          .order('name');
        setShowLinksModal({ invitation, guests: guests || [], publicLink, hasRSVP: true });
      } else {
        setShowLinksModal({ invitation, guests: [], publicLink, hasRSVP: false });
      }
    } catch (error) {
        ...
        <Container>
          <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
            <div>
              ...
              <div className="flex items-center gap-3">
                <span className={`px-2.5 py-1 rounded-full text-xs font-bold ${userPlan === 'pro' ? 'bg-purple-500/30 text-purple-300 border border-purple-400/40' : 'bg-neutral-700 text-neutral-300'}`}>
                  {userPlan === 'pro' ? '⭐ Pro' : '🎁 Gratis'}
                </span>
                <p className="text-neutral-400 text-sm">
                  {userPlan === 'pro'
                    ? `${pricing.noRsvpCost} cr/invitación • ${pricing.perGuestCost} cr/invitado`
                    : `${pricing.noRsvpCost} cr/invitación • ${pricing.perGuestCost} cr/invitado`
                  }
                </p>
              </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-8">
            ...
            <div className="bg-purple-500/20 rounded-2xl p-5 border border-purple-400/30 text-center">
              ...
              <div className="text-purple-200 text-xs mt-1">Créditos</div>
            </div>
          </div>
        </Container>
      </section>
          ...
          <div className="flex gap-3">
            {[
              ...
              <button
                ...
                className={`px-5 py-2.5 rounded-xl font-semibold text-sm transition-all ${activeTab === tab.id ? 'bg-neutral-900 text-white' : 'bg-neutral-100 text-neutral-700 hover:bg-neutral-200'}`}
              >
                {tab.label} ({tab.count})
              </button>
            ))}
          ...
          {filteredInvitations.length === 0 ? (
            <div className="text-center py-20">
              ...
              <h3 className="text-2xl font-display font-bold mb-2">
                {activeTab === 'borradores' ? 'No tienes borradores' : activeTab === 'activas' ? 'No tienes publicadas' : 'No tienes invitaciones aún'}
              </h3>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {filteredInvitations.map((invitation) => {
                ...
                const cost = hasRSVP ? invGuests * pricing.perGuestCost : pricing.noRsvpCost;
                ...
                return (
                  <Card key={invitation.id} className="group">
                    ...
                    <div className="p-5">
                        ...
                        <div className="mb-4 p-3 bg-yellow-50 border border-yellow-200 rounded-xl">
                          ...
                          {hasRSVP ? (
                            <p className="text-xs text-yellow-700 mt-1">
                              ...
                              {invGuests === 0 ? ' • Agrega invitados para publicar' : ` • Costará ${cost} crédito${cost !== 1 ? 's' : ''}`}
                            </p>
                          ) : (
                            <p className="text-xs text-yellow-700 mt-1">🔗 Sin RSVP • Publicar = {pricing.noRsvpCost} créditos</p>
                          )}
                        </div>
                      )}
                      ...
                      {!isDraft && (
                        <div className="grid grid-cols-2 gap-2 mb-4 p-3 bg-neutral-50 rounded-xl">
                          ...
                          <div className="text-center">
                            ...
                            <div className="text-xs text-neutral-500">Créditos</div>
                          </div>
                        </div>
                      )}
                      ...
                      {isDraft && (
                        <>
                          <button onClick={() => handlePublishClick(invitation)} disabled={publishingId === invitation.id} className="w-full mb-2 px-5 py-3 bg-gradient-to-r from-purple-600 to-pink-500 text-white rounded-xl font-bold text-sm shadow-md hover:shadow-lg transition-all disabled:opacity-50">
                          ...
                          <div className="grid grid-cols-2 gap-2">
                            ...
                            {hasRSVP && (
                              <button onClick={() => router.push(`/guests/${invitation.id}`)} className="px-4 py-2 text-sm border-2 border-purple-200 text-purple-700 rounded-xl hover:border-purple-400 hover:bg-purple-50 font-semibold transition-all">👥 Invitados</button>
                            )}
                          </div>
                        </>
                      )}
                            ...
                            <button
                              onClick={async () => {
                                if (hasRSVP) {
                                  ...
                                  setShowLinksModal({ invitation, guests: guests || [], publicLink: `${window.location.origin}/i/${invitation.id}`, hasRSVP: true });
                                } else {
                                  setShowLinksModal({ invitation, guests: [], publicLink: `${window.location.origin}/i/${invitation.id}`, hasRSVP: false });
                                }
                              }}
                      ...
                      <div className="grid grid-cols-2 gap-2 mt-2">
                        <button onClick={() => handleDuplicate(invitation)} className="px-4 py-2 text-sm border-2 border-neutral-200 rounded-xl hover:border-neutral-400 font-semibold transition-all">📋 Duplicar</button>
                        <button onClick={() => handleDelete(invitation.id)} className="px-4 py-2 text-sm border-2 border-red-200 text-red-600 rounded-xl hover:border-red-600 hover:bg-red-50 font-semibold transition-all">🗑️ Eliminar</button>
                      </div>
                    </div>
                  </Card>
                );
            ...
            <div className="bg-white rounded-3xl p-8 max-w-md w-full animate-scale-in">
              ...
              <div className="bg-neutral-50 rounded-2xl p-4 mb-4 space-y-2 text-sm">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="font-semibold">{userPlan === 'pro' ? '⭐ Pro' : '🎁 Gratis'}</span>
                </div>
                <div className="flex justify-between">
              ...
              <div className="bg-purple-50 border border-purple-200 rounded-2xl p-4 mb-6 space-y-2 text-sm">
                {hasRSVP ? (
                  <div className="flex justify-between">
                    <span className="text-purple-700">{invGuests} invitado{invGuests !== 1 ? 's' : ''} × {pricing.perGuestCost} cr:</span>
                    <span className="font-bold text-purple-900">-{cost} créditos</span>
                  </div>
                ) : (
                  <div className="flex justify-between">
                    <span className="text-purple-700">Tarifa fija (sin RSVP):</span>
                    <span className="font-bold text-purple-900">-{pricing.noRsvpCost} créditos</span>
                  </div>
                )}
              ...
              <div className="flex gap-3">
                <button onClick={() => setShowPublishModal(null)} className="flex-1 px-5 py-3 border-2 border-neutral-200 rounded-xl font-semibold text-neutral-600 hover:bg-neutral-50 transition-colors">Cancelar</button>
                <button onClick={confirmPublish} disabled={publishingId !== null} className="flex-1 px-5 py-3 bg-gradient-to-r from-purple-600 to-pink-500 text-white rounded-xl font-bold shadow-md hover:shadow-lg transition-all disabled:opacity-50">
            ...
            <div className="bg-white rounded-3xl p-6 md:p-8 max-w-lg w-full animate-scale-in max-h-[90vh] overflow-y-auto">
              <div className="text-center mb-6">
                ...
                <p className="text-neutral-500 text-sm">
                  {hasRSVP ? `${guests.length} link${guests.length !== 1 ? 's' : ''} personalizado${guests.length !== 1 ? 's' : ''}` : 'Link genérico listo'}
                </p>
              </div>
              ...
              <div className="mb-4">
                ...
                <div className="mt-1 flex items-center gap-2 bg-neutral-50 border border-neutral-200 rounded-xl p-3">
                  ...
                  <button onClick={() => { copyToClipboard(publicLink); alert('✅ Link copiado'); }} className="px-3 py-1.5 bg-neutral-900 text-white rounded-lg text-xs font-bold flex-shrink-0">Copiar</button>
                </div>
              </div>
                      ...
                      return (
                        <div key={guest.id} className="bg-neutral-50 border border-neutral-200 rounded-xl p-3">
                          ...
                          <div className="flex gap-1.5">
                            <button onClick={() => window.open(`https://wa.me/${phone}?text=${encodeURIComponent(waMsg)}`, '_blank')} className="flex-1 px-2.5 py-1.5 bg-green-50 text-green-700 border border-green-200 rounded-lg text-xs font-semibold hover:bg-green-100 transition-colors">📱 WhatsApp</button>
                            <button onClick={() => { copyToClipboard(guestLink); alert(`✅ Link de ${guest.name} copiado`); }} className="flex-1 px-2.5 py-1.5 bg-blue-50 text-blue-700 border border-blue-200 rounded-lg text-xs font-semibold hover:bg-blue-100 transition-colors">🔗 Copiar</button>
                            <button onClick={() => { copyToClipboard(waMsg); alert('✅ Mensaje copiado'); }} className="px-2.5 py-1.5 bg-neutral-100 border border-neutral-200 rounded-lg text-xs font-semibold hover:bg-neutral-200 transition-colors">📋</button>
                          </div>
                        </div>
                      );
                    })}
              ...
              {!hasRSVP && (
                <div className="grid grid-cols-2 gap-3 mt-4">
                  ...
                  >📱 WhatsApp</button>
                  <button onClick={() => { copyToClipboard(publicLink); alert('✅ Copiado'); }} className="px-4 py-3 bg-blue-50 text-blue-600 border-2 border-blue-200 rounded-xl font-semibold hover:bg-blue-100 transition-colors">🔗 Copiar</button>
                </div>
              )}

              <button onClick={() => setShowLinksModal(null)} className="w-full mt-6 px-5 py-3 bg-neutral-900 text-white rounded-xl font-bold hover:bg-neutral-800 transition-colors">Cerrar</button>
            </div>
          </div>
        );
      })()}
