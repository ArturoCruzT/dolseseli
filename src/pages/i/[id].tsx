...
export default function GuestManager() {
  ...
  const handleAddGuest = async () => {
    ...
    // Validar créditos: 1 crédito por invitado
    const userCredits = user?.credits || 0;
    if (guests.length + 1 > userCredits) {
      alert(`⚠️ No tienes créditos suficientes.\n\nTienes ${userCredits} créditos y ya tienes ${guests.length} invitados.\n\nCompra más créditos en la sección de Planes.`);
      return;
    }
  ...
  const handleBulkAdd = async () => {
    ...
    if (totalAfterAdd > userCredits) {
      const available = userCredits - guests.length;
      alert(`⚠️ No tienes créditos suficientes.\n\nTienes ${userCredits} créditos, ya tienes ${guests.length} invitados, e intentas agregar ${lines.length} más.\n\nSolo puedes agregar ${Math.max(0, available)} invitados más.`);
      return;
    }
        ...
        <Container>
          ...
          <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
            ...
            <div className="bg-blue-500/20 backdrop-blur-sm rounded-2xl p-5 border border-blue-400/30 text-center">
              <div className="text-3xl font-bold text-blue-300">{Math.max(0, (user?.credits || 0) - stats.total)}</div>
